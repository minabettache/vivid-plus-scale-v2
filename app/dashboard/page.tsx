      ...
      <aside className={`${styles.sidebar} ${mobileOpen ? styles.sidebarOpen : ''}`}>
        ...
        <div className={styles.locationCard}>
          ...
          <div>
            ...
            <strong>Vivid Lounge Orlando</strong>
          </div>
          <ChevronDown size={16} />
        </div>
        ...
        <nav className={styles.nav}>
          {navigation.map(({ label, icon: Icon, active }) => (
            <button key={label} className={active ? styles.navActive : ''}>
              <Icon size={18} />
              <span>{label}</span>
            </button>
          ))}
        ...
        <header className={styles.topbar}>
          ...
          <div className={styles.topbarActions}>
            <label className={styles.searchBox}>
              <Search size={17} />
              <input placeholder="Search VIVID+" />
            </label>
            <button className={styles.iconButton} aria-label="Notifications">
              <Bell size={19} />
              <i />
            </button>
            <button className={styles.newSaleButton}>
        ...
        <div className={styles.pageBody}>
          <section className={styles.hero}>
            <div>
              ...
              <p>Your business is up 12.4% compared with the previous period.</p>
            </div>
            ...
            {metrics.map(({ label, value, change, detail, icon: Icon, warning }) => (
              <article key={label} className={styles.metricCard}>
                <div className={styles.metricTop}>
                  <span>{label}</span>
                  <div className={warning ? styles.metricIconWarning : styles.metricIcon}><Icon size={19} /></div>
                </div>
                <strong>{value}</strong>
                <div className={styles.metricBottom}>
                  <span className={warning ? styles.warningText : styles.positiveText}>{change}</span>
                  <small>{detail}</small>
                </div>
              </article>
            ))}
          ...
          <section className={styles.mainGrid}>
            <article className={styles.panelLarge}>
              <div className={styles.panelHeader}>
                ...
                <button>View report <ArrowUpRight size={15} /></button>
              </div>
            ...
            <article className={styles.aiPanel}>
              ...
              <div className={styles.insightStat}>
                <TrendingUp size={18} />
                <div><strong>+$486 estimated upside</strong><span>by promoting premium upgrades</span></div>
              </div>
          ...
          <section className={styles.bottomGrid}>
            <article className={styles.tablePanel}>
              <div className={styles.panelHeader}>
                <div><span>Recent activity</span><strong>Latest transactions</strong></div>
                <button>View all <ArrowUpRight size={15} /></button>
              </div>
              <div className={styles.tableWrap}>
                <table>
                  <thead><tr><th>Transaction</th><th>Customer</th><th>Time</th><th>Status</th><th>Amount</th></tr></thead>
                  <tbody>
                    {transactions.map((transaction) => (
                      <tr key={transaction.id}>
                        <td>{transaction.id}</td><td>{transaction.customer}</td><td>{transaction.time}</td>
                        <td><span className={transaction.status === 'Paid' ? styles.statusPaid : styles.statusRefunded}>{transaction.status}</span></td>
            ...
            <article className={styles.inventoryPanel}>
              <div className={styles.panelHeader}>
                <div><span>Inventory alerts</span><strong>Low stock</strong></div>
                <button><Activity size={16} /></button>
              </div>
              <div className={styles.inventoryList}>
                {inventory.map((item) => (
                  <div key={item.name} className={styles.inventoryItem}>
                    <div><strong>{item.name}</strong><span>{item.stock} units remaining</span></div>
                    <div className={styles.progressTrack}><i style={{ width: `${Math.max(10, (item.stock / item.target) * 100)}%` }} /></div>
                  </div>
                ))}
              </div>
