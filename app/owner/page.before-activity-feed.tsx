            ...
            <div className="mb-4 rounded-2xl border border-fuchsia-400/20 bg-fuchsia-500/[0.06] p-4">
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-xs text-zinc-500">
                    {formatMoney(summary.revenueToday)} of {formatMoney(dailyRevenueGoal)}
                  </p>
                </div>

                <span className="text-lg font-black text-fuchsia-300">
                  {Math.min((summary.revenueToday / dailyRevenueGoal) * 100, 100).toFixed(1)}%
                </span>
              </div>
              ...
              <div className="mt-4 h-3 overflow-hidden rounded-full bg-black/40">
                <div
                  ...
                  style={{
                    width: `${Math.min((summary.revenueToday / dailyRevenueGoal) * 100, 100)}%`,
                  }}
                />
              </div>
            </div>
          ...
          <Section title="Business pulse" subtitle="Live operating snapshot">
            <div className="grid gap-3">
              {[
                ...
                <div key={item.label} className="flex items-center justify-between rounded-2xl border border-white/5 bg-black/20 px-4 py-3">
                  <div className="flex items-center gap-3">
                    <div className="rounded-xl bg-white/5 p-2 text-zinc-400"><item.icon className="h-4 w-4" /></div>
                    <span className="text-sm text-zinc-400">{item.label}</span>
                  </div>
                  <span className="text-lg font-black">{whole.format(item.value)}</span>
                </div>
              ))}
            </div>
          </Section>
